...
- 음성 명령 버전: **Commands > Voice Command Version**에서 구/신 방식을 전환한다.
  ...
  - 시선 예비 선택은 카메라 화면 중앙에서 물리 레이캐스트를 발사해 가장 먼저 맞은 기물로 결정한다. FBX 기물에 콜라이더가 없으면 렌더러별 선택용 트리거 콜라이더를 런타임에 자동 생성한다. 가장 앞에 맞은 기물이 적이면 그 뒤의 아군을 대신 선택하지 않으며, 확정 버튼은 현재 레이가 가리키는 예비 선택 기물에만 적용된다.
